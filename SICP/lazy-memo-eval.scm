 ...
 (cond ((self-evaluating? exp) exp)
	   ...
	   ((application? exp) 
		(apply-inner (actual-value (operator exp) env)
		 (operands exp)
		 env))
	   (else (error "Unbound operation -- EVAL" exp))))
...
(define (apply-inner proc args env)
 (cond ((primitive-procedure? proc)
	   ...
	   ((compound-procedure? proc)
		(let ((formal-parameters (procedure-parameters proc)))
		 (eval-sequence (procedure-body proc)
		  (extend-environment formal-parameters
							  (list-of-args formal-parameters args env)
							  (procedure-env proc)))))
...
(define (list-of-args vars args env)
 (if (no-operands? args)
  '()
  (cons (if (lazy? (first-operand vars))
		 (delay-it (first-operand args) env)
		 (eval (first-operand args) env))
   (list-of-args (rest-operands vars)
				 (rest-operands args)
				 env))))
...
(define (make-frame vars vals env)
 (let ((new-frame (cons '() '())))
  (map (lambda (var val) (add-binding-to-frame! (var-name var) (var-value var val env) new-frame)) vars vals)
  new-frame))
 ...
 (define (env-loop env)
  (let ((frame (first-frame env)))
   (let ((vals (scan (frame-variables frame) (frame-values frame) var)))
	(if vals
	 (set-car! vals val)
	 (env-loop (enclosinge-environment env))))))
 (env-loop env))
...
(define (lazy? obj)
 (cond ((not (pair? obj)) false)
	   ((null? (cdr obj)) false)
	   ((eq? (cadr obj) 'lazy) true)
	   ((eq? (cadr obj) 'lazy-memo) true)
	   (else false)))

(define (lazy-memo? obj)
 (and (lazy? obj) (eq? (cadr obj) 'lazy-memo)))
...
(define prompt-input ";;;Lazy-memo-Eval input:")
(define prompt-output ";;;Lazy-memo-Eval value:")
