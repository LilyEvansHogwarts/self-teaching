;;;(lambda? exp) 第一次写写成了(make-procedure (procedure-parameters exp) (procedure-body exp) env)
		...
		(let ((formal-parameters (procedure-parameters procedure)))
		 (eval-sequence (procedure-body procedure)
		                (extend-environment formal-parameters
										    (list-of-args formal-parameters arguments env)
...
(define (list-of-args formal-parameters exps env)
 (if (no-operands? exps)
  '()
  (cons (if (lazy-arg? (first-operand formal-parameters))
...
(define (memo? arg)
 (and (lazy-arg? arg) (eq? (cadr arg) 'lazy-memo)))
