  ...
  (if (tree-empty? tree)
    ...
    (let ((compare-result (compare given-key (tree-key tree))))
      (cond ((= compare-result 0)
	     ...
	     tree)
	    ((= 1 compare-result)
	     (tree-set-right-branch! (tree-insert! (tree-right-branch tree) given-key value compare) tree)
	     tree)
	    ((= -1 compare-result)
	     (tree-set-left-branch! (tree-insert! (tree-left-branch tree) given-key value compare) tree)
	     tree)))))
