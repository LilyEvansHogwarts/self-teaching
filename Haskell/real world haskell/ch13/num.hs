...
--simpleParen x@(BinaryArith op _ _) = "(" ++ prettyShow x ++ ")"
simpleParen x@(BinaryArith op _ _) 
   | op `elem` [Pow,Div,Mul]  = prettyShow x
...
instance (Eq a,Floating a) => Floating (Units a) where
   ...
   asin (Units xa ua) 
      | ua == Symbol "1" = Units (rad2deg $ asin xa) (Number 1)
      | otherwise = error "Units for asin must be empty"
   acos (Units xa ua) 
      | ua == Symbol "1" = Units (rad2deg $ acos xa) (Number 1)
      | otherwise = error "Units for acos must be empty"
   atan (Units xa ua) 
      | ua == Symbol "1" = Units (rad2deg $ atan xa) (Number 1)
